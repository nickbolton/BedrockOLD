...
static NSInteger const kPBCalendarViewControllerVisibleMonths = 5;
static CGFloat kPBCalendarViewControllerItemHeight = 264.0f;
static CGFloat kPBCalendarViewControllerToolbarHeight = 40.0f;
static NSInteger const kPBCalendarViewControllerCalendarTag = 999;

@interface PBCalendarViewController () <UIGestureRecognizerDelegate> {
...
- (void)setupToolbar {
    ...
    [NSLayoutConstraint
     addHeightConstraint:kPBCalendarViewControllerToolbarHeight
     toView:self.toolbar];
...
- (PBListItem *)itemForMonthDate:(NSDate *)monthDate {
    ...
    [PBListItem
     ...
     configure:^(id viewController, PBListItem *item, PBListCell *cell) {
         ...
         PBCalendarView *calendarView =
         (id)[cell.contentView viewWithTag:kPBCalendarViewControllerCalendarTag];
    ...
    item.rowHeight = kPBCalendarViewControllerItemHeight;
...
- (NSArray *)buildFullDataSource {
    ...
    for (NSInteger i = -1; i < kPBCalendarViewControllerVisibleMonths + 1; i++) {
        ...
        if (calendarView == nil) {
            ...
            calendarView.tag = kPBCalendarViewControllerCalendarTag;
    ...
    for (NSInteger i = 0; i < kPBCalendarViewControllerVisibleMonths; i++) {
...
//    NSLog(@"co: %@", NSStringFromCGPoint(scrollView.contentOffset));
    ...
    kPBCalendarViewControllerItemHeight * (kPBCalendarViewControllerVisibleMonths - 2.5);
    CGFloat frontThreshold =
    kPBCalendarViewControllerItemHeight * 1.5;

    CGPoint currentOffset = scrollView.contentOffset;
    if (currentOffset.y < frontThreshold) {

        CGFloat distanceFromThreshold =
        currentOffset.y - kPBCalendarViewControllerItemHeight/2.0f;

        CGFloat offset =
        kPBCalendarViewControllerItemHeight +
        kPBCalendarViewControllerItemHeight/2.0f +
...
- (CGPoint)zeroContentOffset {
    ...
    return
    CGPointMake(0.0f, kPBCalendarViewControllerItemHeight + topSpaceInView);
}
...
- (void)jumpToCurrentMonth {
    ...
    if (monthComponents.month == 1) {
        ...
        contentOffset.y -= kPBCalendarViewControllerItemHeight;
...
- (void)enumerateCalendarViews:(void(^)(PBCalendarView *calendarView, NSInteger index, BOOL *stop))block {
    ...
    for (UIView *cell in self.tableView.visibleCells) {
        ...
        PBCalendarView *calendarView =
        (id)[cell viewWithTag:kPBCalendarViewControllerCalendarTag];
...
- (void)handlePanEnded:(UIGestureRecognizer *)gesture {
    ...
    if ([self.selectedDateRange.startDate isEqualToDate:self.selectedDateRange.endDate.midnight]) {
